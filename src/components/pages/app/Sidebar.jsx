...
const AppSidebar = ({ sidebarExpanded, setSidebarExpanded }) => {
  ...
  return (
    <div className={`${sidebarExpanded ? "w-64" : "w-16"} bg-bg-50 rounded-2xl my-3 mx-3 mr-0 flex flex-col transition-all duration-300 ease-in-out overflow-hidden border border-bd-50`}>
      {/* Header with Logo */}
      <div className={`flex items-center ${sidebarExpanded ? "justify-between pt-[20px] px-3" : "justify-center pt-[20px] px-2 pr-4"} pb-2`}>
        <div className={`flex items-center ${sidebarExpanded ? "px-1" : "pl-2"}`}>
        ...
        <nav className="space-y-1">
          {items.map(({ to, label, icon: Icon }) => (
            <Link
              key={to}
              to={to}
              className={`w-full flex items-center ${sidebarExpanded ? "gap-2 px-2.5" : "justify-center px-2"} py-2 rounded-md transition-colors ${isActive(to) ? "bg-bg-40 text-fg-50" : "hover:bg-bg-30 hover:text-fg-50 text-fg-60"}`}
      ...
      <div className="mt-auto">
        <div className={`relative ${sidebarExpanded ? "p-3" : "p-2"}`} ref={profileDropdownRef}>
          <button
            onClick={() => setShowProfileDropdown(!showProfileDropdown)}
            className={`w-full flex items-center p-2 rounded-lg hover:bg-bg-50/30 focus:bg-bg-50/40 transition-colors duration-150 outline-none ring-0 group ${!sidebarExpanded ? "justify-center" : ""}`}
            tabIndex={0}
            aria-haspopup="true"
          ...
          >
            ...
            {sidebarExpanded && (
              <div className="ml-3 text-left overflow-hidden">
                ...
                <p className="text-xs text-fg-60 truncate">{user?.email || "user@example.com"}</p>
              </div>
            )}
            {sidebarExpanded && (
              <div className="ml-auto">{showProfileDropdown ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</div>
            )}
          </button>

          {showProfileDropdown && sidebarExpanded && (
            <div className="absolute bottom-full left-0 right-0 mb-2 z-50 min-w-[200px] px-2" style={{ pointerEvents: "auto" }}>
              <div className="bg-bg-60 border border-bd-50 shadow-lg rounded-lg py-1 transition-all duration-200 animate-fade-in">
                <div className="px-4 py-2">
                  <p className="text-sm font-semibold text-fg-50 truncate">{user?.name || "User"}</p>
                  <p className="text-xs text-fg-60 truncate">{user?.email || "user@example.com"}</p>
                </div>
                <button
